...
import {
  ...
  ChevronLeft, ChevronRight, GraduationCap, Bot, X, Command, PiggyBank
} from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, isMobileOpen, setIsMobileOpen }) => {
  ...
  const menuItems = [
    ...
    { id: 'accumulation', label: 'Tích sản đầu tư', icon: PiggyBank, shortcut: '6' },
    { id: 'portfolio', label: 'Danh mục SENAI', icon: Briefcase, shortcut: '7' },
    { id: 'knowledge', label: 'Kiến thức', icon: Lightbulb, shortcut: '8' },
  ];
        ...
        {(!isCollapsed || isMobileOpen) && (
          <div className="px-4 mb-4">
            <div className="flex items-center gap-2 px-3 py-2 bg-slate-100 dark:bg-slate-800/50 rounded-lg text-xs text-slate-500 dark:text-slate-400">
              <Command size={12} />
              <span>Nhấn 1-8 để chuyển nhanh</span>
            </div>
          </div>
        )}
        ...
        <nav className="flex-1 px-3 space-y-1 overflow-y-auto custom-scrollbar" role="navigation" aria-label="Menu chính">
          {menuItems.map((item, index) => (
            <button
              ...
              className={`w-full flex items-center ${isCollapsed && !isMobileOpen ? 'justify-center px-0' : 'gap-3 px-4'} py-3 rounded-xl transition-all duration-200 text-sm font-medium group relative focus-visible-ring ${activeTab === item.id
            ...
            >
              ...
              <div className="relative">
                <item.icon
                  size={20}
                  className={`flex-shrink-0 transition-all ${item.highlight ? 'text-indigo-500' : ''
              ...
              {(!isCollapsed || isMobileOpen) && (
                <kbd className={`px-1.5 py-0.5 text-[10px] font-mono rounded transition-all ${activeTab === item.id
